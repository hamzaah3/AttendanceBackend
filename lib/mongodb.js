    ...
    await col.updateOne(
      { _id: existing._id },
      {
        $set: {
          ...
          ...(doc.monthlySalary != null && { monthlySalary: Number(doc.monthlySalary) }),
        },
      }
    );
...
async function mongoGetAttendancesByUserAndDate(userId, date) {
  const db = await getDb();
  const list = await db.collection('attendance').find({ userId, date }).sort({ checkInTime: 1 }).toArray();
  return toDocs(list);
}
...
module.exports = {
  ...
  mongoGetAttendancesByUserAndDate,
